...
const iconMap: { [key: string]: { link?: string, icon: JSX.Element } } = {
  'Accounts Created': { link: 'accounts-created', icon: <FaUsers className="text-3xl fill-primary dark:fill-white" /> },
  ...
  '3rd Places clicked': { link: 'experiences', icon: <MdPlace className="text-3xl fill-primary dark:fill-white" /> },
  'Functions Clicked': { link: 'experiences', icon: <MdEventAvailable className="text-3xl fill-primary dark:fill-white" /> },
  '3rd Places shared': { link: 'experiences', icon: <MdPlace className="text-3xl fill-primary dark:fill-white" /> },
  'Functions Shared': { link: 'experiences', icon: <MdEvent className="text-3xl fill-primary dark:fill-white" /> },

  'Daily Active Users': { icon: <FaUsers className="text-3xl fill-primary dark:fill-white" /> },
  'Dropped In Users': { link: 'neighborhood', icon: <BsArrowDownSquareFill className="text-2xl fill-primary dark:fill-white" /> },
  'Neighborhoods Live': { link: 'neighborhood', icon: <FaMapMarked className="text-3xl fill-primary dark:fill-white" /> },
  'Neighborhoods Active': { link: 'neighborhood', icon: <FaMapMarkedAlt className="text-3xl fill-primary dark:fill-white" /> },
  'Countries active': { icon: <FaGlobeAmericas className="text-2xl fill-primary dark:fill-white" /> },
...
const Dashboard: React.FC = () => {
  ...
  const handleFilterChange = <K extends keyof FilterDashboardQuery>(name: K, value: FilterDashboardQuery[K]) => {
  ...
  return (
    <DefaultLayout>
      ...
      <div className="flex gap-4 mb-8 justify-end">
        <h1 className='text-xl rounded border bg-blue-500 text-gray px-5 py-1 cursor-pointer' onClick={updateStatistics}>{isStatisticsUpdated ? 'Updating..' : 'Refresh'}</h1>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        {dashboardData && dashboardData.map((stat: any, index: any) => (
      ...
      <div className='pt-8'>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
          {liveData && liveData.map((stat: any, index: any) => (
            <Link to={stat.link} key={index}>
              <CardDataStats
                title={stat.title}
                total={stat.total}
