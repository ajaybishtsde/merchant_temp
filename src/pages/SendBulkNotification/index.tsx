  ...
  const colDefs: any = [
    ...
    { field: 'type', headerName: 'Type', flex: 1, sortable: true },
    {
      ...
      cellRenderer: (params: { data: IBulkNotification }) => {
        return <div>{`${params.data.isLink}`}</div>;
      },
    },
    {
      field: 'isStored',
      headerName: 'Is Stored',
      flex: 1,
      sortable: true,
      cellRenderer: (params: { data: IBulkNotification }) => {
        return <div>{`${params.data.isStored}`}</div>;
      },
    },
    {
      field: 'createdAt',
      ...
      cellRenderer: (params: { data: IBulkNotification }) => {
        return (
          <SendBulkNotificationAction
            data={params.data}
            fetchSendBulkNotification={fetchSendBulkNotification}
          />
        );
      },
    },
        ...
        <div className="w-full h-full">
          ...
          <div className="relative z-1 -mt-4">
            {bulkNotification?.count > 0 && (
              <Pagination
                getRequestData={fetchSendBulkNotification}
