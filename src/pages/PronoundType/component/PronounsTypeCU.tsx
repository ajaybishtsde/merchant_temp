...
import { IPronounsType, NewPronounsType, PronounsTypeAPI } from '@/utils/api/pronounstype.api';

interface PronounsTypeCUProps {
    isOpen: boolean;
    toggleModal: () => void;
    fetchLatestData: () => void;
    updateData?: IPronounsType;
}

const PronounsTypeCU: React.FC<PronounsTypeCUProps> = ({ isOpen, toggleModal, fetchLatestData, updateData }) => {
    const { register, handleSubmit, formState: { errors } } = useForm<NewPronounsType>({
    ...
    const onSubmit = async (data: NewPronounsType) => {
        try {
            const formData = {
                ...data,
            };
            // API call for either creating or updating the PronounsType
            const res = updateData?.id
                ? await PronounsTypeAPI.update(updateData.id, formData)
                : await PronounsTypeAPI.create(formData);
    ...
    return (
        <div className='container mx-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
            <BaseModal isOpen={isOpen} toggleModal={toggleModal} heading='PronounsType'>
                <div className="w-full p-4">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                type="text"
                                placeholder="Enter PronounsType Name"
