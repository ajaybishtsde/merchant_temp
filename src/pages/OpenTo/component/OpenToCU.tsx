...
import { IOpenTo, NewOpenTo, OpenToAPI } from '@/utils/api/opento.api';

interface OpenToCUProps {
    isOpen: boolean;
    toggleModal: () => void;
...
const OpenToCU: React.FC<OpenToCUProps> = ({ isOpen, toggleModal, fetchLatestData, updateData }) => {
    const { register, handleSubmit, formState: { errors } } = useForm<NewOpenTo>({
    ...
    const onSubmit = async (data: NewOpenTo) => {
        try {
            const formData = {
                ...data,
            };
            // API call for either creating or updating the OpenTo
            const res = updateData?.id
                ? await OpenToAPI.update(updateData.id, formData)
                : await OpenToAPI.create(formData);
    ...
    return (
        <div className='container mx-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
            <BaseModal isOpen={isOpen} toggleModal={toggleModal} heading='OpenTo'>
