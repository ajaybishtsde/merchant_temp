...
import { IRomanceType, NewRomanceType, RomanceTypeAPI } from '@/utils/api/romancetype.api';

interface RomanceTypeCUProps {
    isOpen: boolean;
    toggleModal: () => void;
    fetchLatestData: () => void;
    updateData?: IRomanceType;
}

const RomanceTypeCU: React.FC<RomanceTypeCUProps> = ({ isOpen, toggleModal, fetchLatestData, updateData }) => {
    const { register, handleSubmit, formState: { errors } } = useForm<NewRomanceType>({
    ...
    const onSubmit = async (data: NewRomanceType) => {
        try {
            const formData = {
                ...data,
            };
            // API call for either creating or updating the romanceType
            const res = updateData?.id
                ? await RomanceTypeAPI.update(updateData.id, formData)
                : await RomanceTypeAPI.create(formData);
    ...
    return (
        <div className='container mx-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
            <BaseModal isOpen={isOpen} toggleModal={toggleModal} heading='RomanceType'>
                <div className="w-full p-4">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                type="text"
                                placeholder="Enter RomanceType Name"
