...
import { INeighborhood, NeighborhoodAPI, NeighborhoodQuery, NeighborhoodResponse } from '@/utils/api/neighborhood.api';
...
const GeoFenceComponent = ({ setMapCoordinates, updateData }: PropsType) => {
  ...
  const [polygons, setPolygons] = useState<{ id: number, isActive: boolean, paths: { lat: number, lng: number }[] }[] | undefined>(undefined);
  const [mapCenter, setMapCenter] = useState<LatLng>(updateData ? { lat: updateData.center.coordinates[1], lng: updateData.center.coordinates[0] } : { lat: 38.9072, lng: -77.0369 }); // Washington, D.C.
