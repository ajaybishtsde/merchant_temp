  ...
  const colDefs: any = [
    {
      ...
      cellRenderer: (params: { data: IAdmin }) => {
        return <p className="first-letter:uppercase">{params?.data?.firstName}</p>;
      },
    },
    {
      field: 'name',
      ...
      filter: true,
      cellRenderer: (params: { data: IAdmin }) => {
        return <p className="first-letter:uppercase">{params?.data?.lastName}</p>;
      },
    },
  ...
  return (
    <div>
      <div className="ag-theme-quartz h-[500px] pb-4">
        <AgGridReact className="w-full" rowData={adminData.data} columnDefs={colDefs} />
      </div>
    </div>
  );
};
